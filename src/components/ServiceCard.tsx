...
export function ServiceCard ({nomeUsuario, modeloAparelho, defeito, status, dataPedido, prazoEntrega, onDelete}: any) {
    return (
        <div className={`border mt-2 ml-4 px-6 py-4 w-80 rounded-lg shadow-md ${status === 'aberto' ? 'bg-green-500' : 'bg-red-600'}`}>
            ...
            <button onClick={onDelete} className="border rounded-lg bg-red-600 text-white px-3 py-1 rounded-md mt-2">
                Deletar
            </button>
        </div>
